...
from keras.layers import Input, Dense, Embedding, LSTM, Concatenate, Reshape, Bidirectional
...
def buildModelNew():
    ...
    # lstm1 = lstm(tf.convert_to_tensor([x1], dtype=tf.float32))
    # lstm2 = lstm(tf.convert_to_tensor([x2], dtype=tf.float32))
    # lstm3 = lstm(tf.convert_to_tensor([x3], dtype=tf.float32))
    # lstm1 = lstm(x1)
    # lstm2 = lstm(x2)
    # lstm3 = lstm(x3)
...
def main():
    ...
    # print(labels[0])
    # print("u1 data len %s " % len(u1_data[0]))
